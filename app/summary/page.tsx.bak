    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-10 md:py-14">
      <section className="mb-8 text-center">
        <div className="hero-pill mb-4">
          <i className="fas fa-chart-line"></i> series summary
        </div>
        <h1 className="hero-heading mb-3 text-[color:var(--primary)]">
      ...
      <section className="mb-10 grid gap-6 lg:grid-cols-2">
        {Object.entries(grouped).map(([team, rows]) => (
          <article
            className="rounded-2xl border border-[color:var(--border)] bg-white/70 p-4 shadow-sm"
            key={team}
          >
            <h2 className="mb-4 text-2xl font-bold text-[color:var(--primary)]">
              {team}
            </h2>
